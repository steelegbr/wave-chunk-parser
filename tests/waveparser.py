...
from wave_chunk_parser.chunks import RiffChunk, CuePoint, CueChunk, ListChunk, LabelChunk, GenericChunk, InfoChunk, NoteChunk
    ...
    for chunk in riff_chunk.sub_chunks:
        ...
        if chunk.get_name == b'fmt ':
            ...
            for cp in chunk.cue_points:
                print(f'  ID {cp.id} : position:{cp.position} , data_chunk_id:{cp.data_chunk_id} , '\
            ...
            for sc in chunk.sub_chunks:
                ...
                if chunk.get_type == b'INFO':
                    ...
                    if sc.get_name == b'labl':
                        ...
                        print(f'  ID {sc.id} : labeled text: sample_length:{sc.sample_length} , purpose:{sc.purpose}')
            ...
            if isinstance(chunk, GenericChunk):
                ...
                if args.dump_lines != '':
                    for num, hex in enumerate(hexdump.hexdump(chunk.datas, 'generator')):
...
def test_set_cue_points(fname_in, fname_out):
    ...
    chunk_list_assoc = riff_chunk.get_chunk(ListChunk.HEADER_LIST, ListChunk.HEADER_ASSOC)
    ...
    chunk_list_info.replace_chunk(InfoChunk('ICRD', datetime.now().strftime("%Y-%m-%d")))
...
def main():
    ...
    # commands parser
    #
    parser = ArgumentParser(description='Parse Wave file',\
    ...
    parser.add_argument('--dump-lines', '-n', default='2', help='limit number of lines for datas dumped  (default:%(default)s)')
